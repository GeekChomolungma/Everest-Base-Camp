...
// Initializer
func (p *WebSocketClientBase) Init(host string, wsPath string) *WebSocketClientBase {
	p.host = host
	p.path = wsPath
...
func (p *WebSocketClientBase) InitWithFeedPath(host string, wsPath string) *WebSocketClientBase {
	p.Init(host, wsPath)
	p.path = feedPath
	return p
}
	...
	for {
		...
		case <-p.ticker.C:
			...
			if elapsedSecond > ReconnectWaitSecond {
				applogger.Info("WebSocket Close...")
				p.Close()
			}
		}
	}
